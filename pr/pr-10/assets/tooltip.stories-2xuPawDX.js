...
`),ht=(t,e,o)=>mt(a`<cosmoz-tooltip-content>
		...
		</cosmoz-tooltip-content>`,t),te=(t,e)=>{const{for:o,heading:n,description:s,placement:i="top",delay:m=300}=e,v=X();J(()=>{if(!o)return;const r=t.getRootNode(),b=r.adoptedStyleSheets??[];b.includes(tt)||(r.adoptedStyleSheets=[...b,tt]);const c=document.createElement("div");c.setAttribute("popover","manual"),c.setAttribute("role","tooltip"),c.classList.add("cosmoz-tooltip-popover"),t.after(c),v.current=c,ht(c,n,s);const f=`[name="${o}"]`,w=`--tooltip-anchor-${o}`;let $;const B=g=>{clearTimeout($),g.style.anchorName=w,c.style.positionAnchor=w,c.style.positionArea=i,$=window.setTimeout(()=>c.showPopover(),m)},h=()=>{clearTimeout($),c.hidePopover()},l=g=>{const y=g.target.closest?.(f);y&&B(y)},u=g=>{const y=g.target.closest?.(f);if(!y)return;const at=g.relatedTarget;at&&y.contains(at)||h()},d=g=>{const y=g.target.closest?.(f);y&&B(y)},p=g=>{g.target.closest?.(f)&&h()},it=()=>{h()};return r.addEventListener("pointerover",l),r.addEventListener("pointerout",u),r.addEventListener("focusin",d),r.addEventListener("focusout",p),r.addEventListener("keydown",it),()=>{clearTimeout($),r.removeEventListener("pointerover",l),r.removeEventListener("pointerout",u),r.removeEventListener("focusin",d),r.removeEventListener("focusout",p),r.removeEventListener("keydown",it),c.hidePopover(),c.remove(),v.current=void 0}},[o,i,m]),J(()=>{!o||!v.current||ht(v.current,n,s)},[n,s,o])},ee=k`
	:host {
...
`,oe=t=>{const{heading:e,description:o,for:n,placement:s="top",delay:i=300}=t,m=X(),v=X(),r=ut(()=>{clearTimeout(v.current),v.current=window.setTimeout(()=>{m.current?.showPopover()},i)},[i]),b=ut(()=>{clearTimeout(v.current),m.current?.hidePopover()},[]);return J(()=>{if(n)return;const c=f=>{const w=f.relatedTarget;w&&t.contains(w)||b()};return t.addEventListener("pointerover",r),t.addEventListener("pointerout",c),()=>{t.removeEventListener("pointerover",r),t.removeEventListener("pointerout",c)}},[n,r,b]),te(t,{for:n,heading:e,description:o,placement:s,delay:i}),n?q:a`
		<slot @focusin=${r} @focusout=${b} @keydown=${b}></slot>
		<div
