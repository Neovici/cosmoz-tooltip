...
`,Dt=["variant","size","disabled","full-width","type"],jt=t=>{const e=t.hasAttribute("disabled"),o=t.getAttribute("type")||"button";return r`
	...
	`};customElements.define("cosmoz-button",W(jt,{observedAttributes:Dt,styleSheets:[K,Nt],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const $=t=>t??Q;function w(t,e,o){return t?e(t):o?.(t)}const Gt=({slot:t,title:e,className:o,width:n="24",height:s="24",styles:i}={})=>r`
  <svg
    slot=${$(t)}
...
`,Jt=t=>{const{heading:e,description:o,for:n,placement:s="top",delay:i=300}=t,h=V(),g=V(),l=rt(()=>{clearTimeout(g.current),g.current=window.setTimeout(()=>{h.current?.showPopover()},i)},[i]),f=rt(()=>{clearTimeout(g.current),h.current?.hidePopover()},[]);return qt(t,{for:n,heading:e,description:o,placement:s,delay:i}),n?r``:r`
		<slot
			@pointerenter=${l}
			@pointerleave=${f}
			@focusin=${l}
