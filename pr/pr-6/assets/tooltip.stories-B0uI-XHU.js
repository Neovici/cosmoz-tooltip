...
`,Ot=["variant","size","disabled","full-width","type"],Pt=t=>{const e=t.hasAttribute("disabled"),o=t.getAttribute("type")||"button";return a`
	...
	`};customElements.define("cosmoz-button",st(Pt,{observedAttributes:Ot,styleSheets:[Mt,Ht],shadowRootInit:{mode:"open",delegatesFocus:!0}}));const f=t=>t??v;function U(t,e,o){return t?e(t):o?.(t)}const It=({slot:t,title:e,className:o,width:n="24",height:s="24",styles:i}={})=>a`
  <svg
    slot=${f(t)}
    class=${`delete-icon ${o??""}`}
    viewBox="0 0 24 24"
...
`,Gt=t=>{const{heading:e,description:o,for:n,placement:s="top",delay:i=300}=t,l=J(),w=J(),p=D(()=>{clearTimeout(w.current),w.current=window.setTimeout(()=>{l.current?.showPopover()},i)},[i]),m=D(()=>{clearTimeout(w.current),l.current?.hidePopover()},[]),S=D(()=>n?t.getRootNode()?.querySelector(`[name="${n}"]`):null,[n,t]);yt(()=>{if(!n)return;const r=S();if(r)return r.style.anchorName="--tooltip-anchor-external",l.current&&(l.current.style.positionAnchor="--tooltip-anchor-external"),r.addEventListener("mouseenter",p),r.addEventListener("mouseleave",m),r.addEventListener("focusin",p),r.addEventListener("focusout",m),()=>{r.removeEventListener("mouseenter",p),r.removeEventListener("mouseleave",m),r.removeEventListener("focusin",p),r.removeEventListener("focusout",m),r.style.anchorName="",l.current&&(l.current.style.positionAnchor="")}},[n,S,p,m]);const _=`position-area: ${s};`;return a`
		${n?v:a`
					<slot
						@mouseenter=${p}
						@mouseleave=${m}
		...
		<div
			...
			${Ft(r=>{l.current=r})}
		>
			${e?a`<strong class="title">${e}</strong>`:v}
			${o?a`<p class="description">${o}</p>`:v}
